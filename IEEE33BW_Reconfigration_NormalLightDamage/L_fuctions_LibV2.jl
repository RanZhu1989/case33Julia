...
function paraInit(DataPath,startPoint,endPoint)
    ...
    #生成标准情况下线路(i,j)U(j,i)元组
    global undirectedLines=ijjiLine(lines)
    #生成标准情况下的逆序线路(j,i)元组
    global reverseLines=ij2jiReverse(lines)
    #生成标准情况下所有线路(i,j,t) (j,i,t)元组 #ATTITION!在重载函数时需要重新设置  #FIXME:还没有设置
    global tsLines=ijjiAllTime1Pair(lines,points) 
    #生成标准情况下的(k,i,j,t)U(k,j,i,t) 其中k对应k个节点 #ATTITION!在重载函数时需要重新设置  #FIXME:还没有设置
    global mulitCommodFlowPairs=kijtPairs(tsLines,rootFreeNodes)
    #生成(i,j)->NO.line的字典
    ...
    global time0LineIJTPair=lines2TimePair(lines,points,0)
    ...
    global tsMTalive=mkEquipITpair("MT",points,DataPath,"List")
    global tsPValive=mkEquipITpair("PV",points,DataPath,"List")
    global tsWTalive=mkEquipITpair("WT",points,DataPath,"List")
    # global dicTSmtAlive=mkEquipITpair("MT",points,DataPath,"Dict")
    # global dicTSpvAlive=mkEquipITpair("PV",points,DataPath,"Dict")
    # global dicTSwtAlive=mkEquipITpair("WT",points,DataPath,"Dict")
...
function paraRolling(startPoint,endPoint,DataPath)
    ...
    #(i,t) 节点-时间 二元组 t指规划尺度上的相对时刻 #ATTITION!在重载函数时需要重新设置
    global itPair=time1itPair(numNode,points)
    #生成标准情况下所有线路(i,j,t) (j,i,t)元组 #ATTITION!在重载函数时需要重新设置 
    global tsLines=ijjiAllTime1Pair(lines,points) 
    #生成标准情况下的(k,i,j,t)U(k,j,i,t) 其中k对应k个节点 #ATTITION!在重载函数时需要重新设置  #FIXME:还没有设置
    global mulitCommodFlowPairs=kijtPairs(tsLines,rootFreeNodes)
    #生成规划尺度上t时刻存活的线路(i,j,t)元组 #ATTITION!在重载函数时需要重新设置
...
function kijtPairs(tsLines,rootFreeNodes)
    #生成有向多商品流模型用的(k,i,j,t) U (k,j,i,t) 序列列表
    #标准情况下 不考虑故障
    #已测试
    result=[]
    for ijt in tsLines
        for n in rootFreeNodes
            push!(result,(n,ijt[1],ijt[2],ijt[3]))
        end
    end
    return result
end

function mcITpairs(itPair)
    #McCormick包约束所用 不含root及MT节点的所有节点组成的 存活的 it pair
    #已测试
    result=[]
    for it in itPair
        if !((it in tsMTalive)||(it[1]==1))
            push!(result,it)
        end
    end
    return Tuple(result)
...
function lines2TimePair(lines,points,start)
    #已测试
    #生成(i,j,t)组成的列表 其中(i,j)包含了所有线路 t是相对规划时刻 
...
function ijjiAllTime1Pair(lines,points)
    #生成所有线路的(i,j,t) (j,i,t) 列表
    #已测试
    result=[]
    for line in lines
        for t in 1:points
            temp1=(line[1],line[2],t)
            temp2=(line[2],line[1],t)
            push!(result,temp1)
            push!(result,temp2)
        end
    end
...
function findDMCijjiNeighborPairs(i,undirectedLines,mode)
    #用于Directed Multi-Commodity Flow 中寻找邻居生成节点对
    #针对标准情况
    #在 ij ji pair 中寻找
    #已测试
    result=[]
    for line in undirectedLines
        if mode=="forward"
            if i==line[1]
                temp=(line[1],line[2])
                push!(result,temp)
...
function mkEquipITpair(type,points,DataPath,mode)
    #已测试
    #创建特定类型设备的(i,t)组成的字典或元组 即在t时刻i节点上存活
