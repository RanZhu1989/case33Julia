...
function dpSolverReconfiguraiton33Bus(typeOptimizer,mode,maxTime,reGap,JDCflag,ZLBflag)
    ...
    #表示节点负载率情况
    @variable(bus33Reconfiguration,0<=loadRate[it in itPair]<=1,base_name="节点负载率loadRate",start=1)

    #ST约束辅助变量beta
    @variable(bus33Reconfiguration,beta[ijt in ijtjit1Pair],Bin,base_name="ST约束辅助变量beta")
